...
class DriveUploader(private val context: Context) {
    ...
    suspend fun uploadFileAndRecordWithSharing(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                // 📁 トップのフォルダ ShareFileBCApp を取得 or 作成
                val appFolderId = getOrCreateFolder(driveService, "ShareFileBCApp", "root")

                // 🧠 フォルダ共有を1回だけ追加（recipientEmail ごとに）
                addFolderPermissionOnce(driveService, appFolderId, recipientEmail)
    ...
    private val sharedFolderCache = mutableSetOf<Pair<String, String>>() // (folderId, email)

    private fun addFolderPermissionOnce(drive: Drive, folderId: String, email: String) {
        val key = folderId to email
        if (sharedFolderCache.contains(key)) return
        try {
            ...
            drive.permissions().create(folderId, permission)
                ...
                .execute()
            sharedFolderCache.add(key)
        } catch (e: Exception) {
            Log.e("DriveUploader", "Failed to share folder", e)
        }
    }
}
